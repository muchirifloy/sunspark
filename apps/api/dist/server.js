...
function requireServiceToken(request, _response, next) {
    if (!adminApiToken) {
...
function fallbackProductOption(row) {
    return {
        id: "",
        productId: row.id,
...
function mapProduct(row, images = [], options = []) {
...
async function assertUniqueProductSlug(slug, currentId) {
    const slugRows = await query("SELECT id FROM products WHERE slug = ? LIMIT 1", [slug]);
    if (slugRows[0] && slugRows[0].id !== currentId) {
        throw new HttpError(409, "A product with that name already exists.");
    }
}
async function imagesForProducts(productIds) {
    if (!productIds.length)
        return new Map();
    const rows = await query(`SELECT * FROM product_images WHERE product_id IN (${productIds.map(() => "?").join(",")})
     ORDER BY is_primary DESC, sort_order ASC, created_at ASC`, productIds);
...
async function imagesForCategories(categoryIds) {
    if (!categoryIds.length)
        return new Map();
    const rows = await query(`SELECT * FROM category_images WHERE category_id IN (${categoryIds.map(() => "?").join(",")})
     ORDER BY is_primary DESC, sort_order ASC, created_at ASC`, categoryIds);
    return rows.reduce((map, row) => {
...
async function optionsForProducts(productIds) {
    if (!productIds.length)
        return new Map();
    const rows = await query(`SELECT * FROM product_options WHERE product_id IN (${productIds.map(() => "?").join(",")})
     ORDER BY is_default DESC, sort_order ASC, created_at ASC`, productIds);
    return rows.reduce((map, row) => {
        const list = map.get(row.product_id) ?? [];
        list.push(row);
        map.set(row.product_id, list);
...
async function ensureDefaultOption(connection, productId, input) {
    ...
    const rows = await (connection
        ...
        : query("SELECT id FROM product_options WHERE product_id = ? AND is_default = TRUE LIMIT 1", [productId]));
    const existing = rows[0];
    if (existing) {
        await runner.query("UPDATE product_options SET label = ?, selling_unit = ?, price_cents = ?, compare_at_cents = ?, cost_cents = ?, sort_order = 0 WHERE id = ?", [unitLabel(input.sellingUnit), input.sellingUnit, input.priceCents, input.compareAtCents ?? null, input.costCents, existing.id]);
        return existing.id;
    }
    const optionId = id("opt");
    await runner.query(`INSERT INTO product_options
     (id, product_id, label, selling_unit, price_cents, compare_at_cents, cost_cents, stock_multiplier, is_default, sort_order)
     VALUES (?, ?, ?, ?, ?, ?, ?, 1, TRUE, 0)`, [optionId, productId, unitLabel(input.sellingUnit), input.sellingUnit, input.priceCents, input.compareAtCents ?? null, input.costCents]);
    return optionId;
}
async function syncProductOptions(connection, productId, options, deleteOptionIds = []) {
    ...
    const defaults = await connection.query("SELECT id FROM product_options WHERE product_id = ? AND is_default = TRUE ORDER BY sort_order ASC", [productId]);
...
app.patch("/admin/settings", asyncRoute(async (request, response) => {
    ...
    await execute(`INSERT INTO site_settings (id, store_name, support_email, report_email, whatsapp_phone, currency)
     ...
     ON DUPLICATE KEY UPDATE
       ...
       currency = VALUES(currency)`, [input.storeName, input.supportEmail, input.reportEmail, input.whatsappPhone, input.currency]);
...
app.get("/categories/:slug", asyncRoute(async (request, response) => {
    const rows = await query("SELECT * FROM categories WHERE slug = ? AND is_active = TRUE LIMIT 1", [request.params.slug]);
...
app.get("/admin/sale-products", asyncRoute(async (request, response) => {
    ...
    const rows = await query(`SELECT p.id, p.name, p.price_cents, p.compare_at_cents, p.cost_cents, p.selling_unit,
...
app.get("/products/by-slugs", asyncRoute(async (request, response) => {
    ...
    const rows = await query(`SELECT p.*, c.name AS category_name, c.slug AS category_slug
     ...
     ORDER BY p.updated_at DESC`, slugs);
    const [imageMap, optionMap] = await Promise.all([
        imagesForProducts(rows.map((row) => row.id)),
        optionsForProducts(rows.map((row) => row.id))
    ]);
...
app.get("/products/:slug", asyncRoute(async (request, response) => {
    const rows = await query(`SELECT p.*, c.name AS category_name, c.slug AS category_slug
     FROM products p JOIN categories c ON c.id = p.category_id
     WHERE p.slug = ? LIMIT 1`, [request.params.slug]);
...
app.get("/admin/products/:id", asyncRoute(async (request, response) => {
    const rows = await query(`SELECT p.*, c.name AS category_name, c.slug AS category_slug
     FROM products p JOIN categories c ON c.id = p.category_id
     WHERE p.id = ? LIMIT 1`, [request.params.id]);
    if (!rows[0])
        throw new HttpError(404, "Product not found.");
...
app.get("/products/:id/related", asyncRoute(async (request, response) => {
    ...
    const rows = await query("SELECT category_id FROM products WHERE id = ? LIMIT 1", [productId]);
    if (!rows[0])
        throw new HttpError(404, "Product not found.");
...
app.get("/admin/stats", asyncRoute(async (_request, response) => {
    const [products, orders, customers, lowStock] = await Promise.all([
        ...
        query("SELECT COUNT(*) AS count FROM products WHERE stock_quantity <= low_stock_threshold")
    ]);
    response.json({
...
app.get("/admin/sales-summary", asyncRoute(async (request, response) => {
    ...
    const rows = await query(`SELECT ${dateExpression} AS bucket,
...
app.get("/admin/dashboard-overview", asyncRoute(async (_request, response) => {
    const [salesRows, customerRows, categoryRows, productRows, recentOrders, inventoryRows, sms] = await Promise.all([
       ...
       LIMIT 6`),
        query(`SELECT oi.product_id AS productId, oi.product_name AS name,
         COALESCE(SUM(oi.total_cents), 0) AS salesCents,
         COALESCE(SUM(oi.quantity), 0) AS units
       FROM order_items oi
       INNER JOIN orders o ON o.id = oi.order_id
       ...
       ORDER BY salesCents DESC
       LIMIT 5`),
        query(`SELECT id, order_number AS orderNumber, customer_name AS customerName,
...
app.post("/admin/categories", asyncRoute(async (request, response) => {
    ...
    await transaction(async (connection) => {
        await connection.query("INSERT INTO categories (id, name, slug, description, sort_order, is_active) VALUES (?, ?, ?, ?, ?, ?)", [categoryId, input.name, input.slug, input.description ?? null, input.sortOrder, input.isActive]);
        for (const image of input.images) {
            await connection.query("INSERT INTO category_images (id, category_id, url, alt, is_primary, sort_order) VALUES (?, ?, ?, ?, ?, ?)", [id("cim"), categoryId, image.url, image.alt ?? null, image.isPrimary, image.sortOrder]);
        }
    });
...
app.patch("/admin/categories/:id", asyncRoute(async (request, response) => {
    ...
    await transaction(async (connection) => {
        await connection.query("UPDATE categories SET name = ?, slug = ?, description = ?, sort_order = ?, is_active = ? WHERE id = ?", [input.name, input.slug, input.description ?? null, input.sortOrder, input.isActive, request.params.id]);
        ...
        const countRows = await connection.query("SELECT COUNT(*) AS count FROM category_images WHERE category_id = ?", [request.params.id]);
        const existingCount = Number(countRows[0]?.count ?? 0);
        for (const [index, image] of input.images.entries()) {
            await connection.query("INSERT INTO category_images (id, category_id, url, alt, is_primary, sort_order) VALUES (?, ?, ?, ?, ?, ?)", [id("cim"), request.params.id, image.url, image.alt ?? null, existingCount === 0 && index === 0, existingCount + index]);
        }
...
app.delete("/admin/categories/:id", asyncRoute(async (request, response) => {
    const linkedRows = await query(`SELECT COUNT(*) AS count
     FROM draft_document_items ddi
     INNER JOIN products p ON p.id = ddi.product_id
     WHERE p.category_id = ?`, [request.params.id]);
    ...
    await transaction(async (connection) => {
        ...
        const result = await connection.query("DELETE FROM categories WHERE id = ?", [request.params.id]);
...
app.post("/admin/products", asyncRoute(async (request, response) => {
    ...
    await transaction(async (connection) => {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        for (const image of input.images) {
            await connection.query("INSERT INTO product_images (id, product_id, url, alt, is_primary, sort_order) VALUES (?, ?, ?, ?, ?, ?)", [id("pim"), productId, image.url, image.alt ?? null, image.isPrimary, image.sortOrder]);
        }
    });
...
app.patch("/admin/products/:id", asyncRoute(async (request, response) => {
    ...
    await transaction(async (connection) => {
        await connection.query(`UPDATE products SET name = ?, slug = ?, brand = ?, category_id = ?, short_description = ?,
       ...
       seo_keywords = ? WHERE id = ?`, [
        ...
        const countRows = await connection.query("SELECT COUNT(*) AS count FROM product_images WHERE product_id = ?", [request.params.id]);
        const existingCount = Number(countRows[0]?.count ?? 0);
        for (const [index, image] of input.images.entries()) {
            await connection.query("INSERT INTO product_images (id, product_id, url, alt, is_primary, sort_order) VALUES (?, ?, ?, ?, ?, ?)", [id("pim"), request.params.id, image.url, image.alt ?? null, existingCount === 0 && index === 0, existingCount + index]);
        }
        if (input.primaryImageId) {
...
app.delete("/admin/products/:id", asyncRoute(async (request, response) => {
    const draftRows = await query("SELECT COUNT(*) AS count FROM draft_document_items WHERE product_id = ?", [request.params.id]);
    ...
    const result = await execute("DELETE FROM products WHERE id = ?", [request.params.id]);
    if (!result.affectedRows)
        throw new HttpError(404, "Product not found.");
    response.status(204).send();
}));
app.post("/admin/campaigns", asyncRoute(async (request, response) => {
    ...
    await execute("INSERT INTO campaigns (id, title, description, image_url, badge, offer_label, cta_label, cta_url, ends_at, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [campaignId, input.title, input.description ?? null, input.imageUrl ?? null, input.badge ?? null, input.offerLabel ?? null, input.ctaLabel ?? null, input.ctaUrl ?? null, input.endsAt ? new Date(input.endsAt) : null, input.isActive]);
...
app.get("/admin/draft-documents", asyncRoute(async (request, response) => {
    ...
    const documents = await query(`SELECT id, reference, kind, status, order_id AS orderId, customer_name AS customerName,
...
app.get("/admin/draft-documents/:id", asyncRoute(async (request, response) => {
    const documents = await query(`SELECT id, reference, kind, status, order_id AS orderId, customer_name AS customerName,
      customer_email AS customerEmail, customer_phone AS customerPhone, payment_method AS paymentMethod,
      subtotal_cents AS subtotalCents, total_cents AS totalCents, created_at AS createdAt, updated_at AS updatedAt
     FROM draft_documents WHERE id = ? LIMIT 1`, [request.params.id]);
    if (!documents[0])
...
app.post("/admin/draft-documents/:id/finalize", asyncRoute(async (request, response) => {
    ...
    if (!document || document.status !== "DRAFT")
        throw new HttpError(400, "This invoice is already finalized or unavailable.");
    if (document.kind !== "INVOICE")
        throw new HttpError(400, "Quotations do not update stock.");
    const items = await query("SELECT * FROM draft_document_items WHERE document_id = ?", [request.params.id]);
    if (!items.length)
...
app.post("/auth/login", asyncRoute(async (request, response) => {
    ...
    const rows = await query("SELECT * FROM users WHERE email = ? LIMIT 1", [input.email]);
    const user = rows[0];
...
app.get("/users/:id", asyncRoute(async (request, response) => {
    const rows = await query("SELECT id, name, email, phone, role, created_at FROM users WHERE id = ? LIMIT 1", [request.params.id]);
    if (!rows[0])
...
app.post("/auth/forgot-password", asyncRoute(async (request, response) => {
    ...
    if (rows[0]) {
        ...
        await execute("INSERT INTO password_reset_tokens (id, user_id, token_hash, expires_at) VALUES (?, ?, ?, ?)", [id("rst"), rows[0].id, hashToken(token), expires]);
...
app.post("/orders/checkout", asyncRoute(async (request, response) => {
    ...
    const order = await transaction(async (connection) => {
        ...
        const lines = input.items.map((item) => {
            const product = products.get(item.productId);
            if (!product)
                throw new HttpError(400, "One cart product is not available.");
            const options = optionMap.get(product.id) ?? [];
            const selectedOption = options.find((option) => option.id === item.productOptionId) ??
...
app.get("/admin/orders/pending-count", asyncRoute(async (_request, response) => {
    const rows = await query("SELECT COUNT(*) AS count FROM orders WHERE status = 'PENDING'");
...
app.get("/admin/orders/:id", asyncRoute(async (request, response) => {
    const orders = await query(`SELECT
       ...
       status, payment_method AS paymentMethod, payment_status AS paymentStatus,
       subtotal_cents AS subtotalCents, total_cents AS totalCents, created_at AS createdAt, updated_at AS updatedAt
     FROM orders WHERE id = ? LIMIT 1`, [request.params.id]);
    if (!orders[0])
        throw new HttpError(404, "Order not found.");
    const [items, invoices] = await Promise.all([
        query(`SELECT id, order_id AS orderId, product_id AS productId, product_option_id AS productOptionId,
