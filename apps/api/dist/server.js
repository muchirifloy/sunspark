...
app.get("/admin/customers", asyncRoute(async (request, response) => {
    ...
    const rows = await query(`SELECT id, name, email, phone, role, created_at AS createdAt, updated_at AS updatedAt
     FROM users WHERE ${where.join(" AND ")} ORDER BY created_at DESC LIMIT 200`, values);
    response.json(rows);
}));
