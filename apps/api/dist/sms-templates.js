 ...
 */
export function isTransactional(purpose) {
 ...
 */
export function senderIdFor(senderIds, purpose) {
 ...
 */
export function smsRecipient(value) {
...
export function smsRecipients(values) {
    const seen = new Set();
 ...
 */
export function toGsm7(message) {
    let out = "";
    for (const character of message.replace(/\r\n/g, "\n")) {
        const mapped = SUBSTITUTIONS[character] ?? character;
        for (const candidate of mapped) {
            if (GSM7_BASIC.includes(candidate) || GSM7_EXTENDED.includes(candidate))
                out += candidate;
        }
    }
...
export function gsm7Length(message) {
    let length = 0;
    for (const character of message)
        length += GSM7_EXTENDED.includes(character) ? 2 : 1;
    return length;
}
...
function firstName(value) {
    ...
    if (PLACEHOLDER_NAMES.has(trimmed.toLowerCase().replace(/\s+customer$/, "")))
        return "";
 ...
 */
export function smsSignature(brand) {
 ...
 */
export function orderSms(purpose, brand, context) {
 ...
 */
export function composedSms(body, brand) {
