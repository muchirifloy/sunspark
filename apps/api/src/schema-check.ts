...
const notAColumn =
  /^(PRIMARY\s+KEY|UNIQUE\s+(KEY|INDEX)|KEY|INDEX|FULLTEXT|SPATIAL|CONSTRAINT|FOREIGN\s+KEY|CHECK)\b/i;
...
function topLevelParts(body: string) {
  const parts: string[] = [];
  let depth = 0;
  let current = "";
...
export function parseExpectedSchema(sql: string): ExpectedSchema {
  ...
  while ((match = createTable.exec(source))) {
    ...
    if (already) for (const column of columns) already.add(column);
    else expected.set(table, columns);
  }
...
export function compareSchema(expected: ExpectedSchema, actual: Map<string, Set<string>>): SchemaDrift {
  ...
  for (const [table, columns] of expected) {
    ...
    for (const column of columns) {
      if (!live.has(column)) missingColumns.push({ table, column });
    }
  }
...
async function liveSchema(): Promise<Map<string, Set<string>>> {
  const rows = await query<{ TABLE_NAME?: string; table_name?: string; COLUMN_NAME?: string; column_name?: string }>(
...
export async function schemaDrift(): Promise<SchemaDrift> {
  return compareSchema(parseExpectedSchema(readFileSync(migrationPath(), "utf8")), await liveSchema());
}

export function driftSummary(drift: SchemaDrift) {
  const lines: string[] = [];
  if (drift.missingTables.length) lines.push(`missing tables: ${drift.missingTables.join(", ")}`);
  for (const { table, column } of drift.missingColumns) lines.push(`missing column: ${table}.${column}`);
  return lines;
}
