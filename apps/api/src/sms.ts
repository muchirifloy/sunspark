...
export type SmsConfiguration = {
   ...
   * transactional traffic, one for promotional. Only the sender ID differs.
   */
  transactionalSenderId: string;
  promotionalSenderId: string;
  transactionalEnabled: boolean;
  marketingEnabled: boolean;
  /** Every send is composed and logged but never handed to the network. */
  dryRun: boolean;
};
...
export function describeSmsCode(code: string | number | null | undefined) {
  const key = String(code ?? "").trim();
  return RETURN_CODES[key] ?? (key ? `Celcom returned code ${key}.` : "Celcom returned no code.");
}

type SettingsRow = { store_name: string | null; whatsapp_phone: string | null };

let brandCache: { value: SmsBrand; expires: number } | null = null;
 ...
 */
export async function smsBrand(): Promise<SmsBrand> {
  if (brandCache && brandCache.expires > Date.now()) return brandCache.value;

  let settings: SettingsRow | undefined;
  try {
    const rows = await query<SettingsRow>("SELECT store_name, whatsapp_phone FROM site_settings WHERE id = 'default' LIMIT 1");
    settings = rows[0];
  } catch (error) {
  ...
  const value: SmsBrand = { name, phone: localPhone(phone), website };
...
async function postJson(url: string, body: Record<string, unknown>, timeoutMs = 15_000, attempts = 2) {
...
export async function sendSms(input: {
  ...
  const empty = (skipped: string): SmsSendOutcome => ({ sent: 0, failed: 0, skipped, segments: 0, message: "", results: [] });
  ...
  if (!isTransactional(input.purpose) && !configuration.marketingEnabled) return empty("Promotional SMS is switched off.");
  ...
  try {
    ...
    for (const chunk of chunked(recipients, 100)) {
      const body = await postJson(configuration.sendUrl, {
        ...
        message,
        pass_type: "plain"
      }) as { responses?: CelcomResponseRow[] };
 ...
 */
export function queueOrderSms(purpose: SmsPurpose, context: OrderSmsContext & { phone: string | null | undefined; orderId?: string | null }) {
  ...
  void (async () => {
    ...
    if (outcome.skipped) console.warn("Order SMS skipped", { purpose, orderNumber: context.orderNumber, reason: outcome.skipped });
...
async function fetchBalance(configuration: SmsConfiguration) {
  try {
...
export async function smsReport(limit = 200) {
  ...
  const [rows, byStatus, byChannel] = await Promise.all([
    query<SmsMessageRow>("SELECT * FROM sms_messages ORDER BY created_at DESC LIMIT ?", [capped]),
    query<{ status: string; count: number }>(
      "SELECT status, COUNT(*) AS count FROM sms_messages WHERE created_at >= ? GROUP BY status",
      [since]
    ),
    // Segments, not messages: a long message is billed more than once, so this is what
    // actually draws the credit balance down.
    query<{ channel: string; segments: number }>(
      "SELECT channel, COALESCE(SUM(segments), 0) AS segments FROM sms_messages WHERE created_at >= ? GROUP BY channel",
      [since]
    )
  ]);
...
export async function smsDashboardSummary() {
  ...
  try {
    ...
    const rows = await query<{ bucket: string; messages: number; segments: number; failed: number }>(
      `SELECT
    ...
    const total = (key: "messages" | "segments" | "failed") => rows.reduce((sum, row) => sum + Number(row[key] ?? 0), 0);
...
export async function refreshSmsDeliveryReports(limit = 25) {
  if (!smsConfiguration()) return { checked: 0, updated: 0 };

  const pending = await query<{ id: string; provider_message_id: string }>(
    `SELECT id, provider_message_id FROM sms_messages
  ...
  for (const row of pending) {
    ...
    await execute(
      `UPDATE sms_messages
       ...
       WHERE id = ?`,
      [status, report ? JSON.stringify(report).slice(0, 255) : null, status === "DELIVERED" ? new Date() : null, row.id]
    );
